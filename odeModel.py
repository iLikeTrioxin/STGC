...
A = 1e5            # pre-exponential (units depend on rate law)
E_A = 92000             # activation energy J/mol
...
C_SO3a[1:] = (Fr/ZONE_VOLUMES[1:]) * (C_SO3[0:-1] - C_SO3[1:]) + rate[1:]
...
C_SO3a[0] = ( (Fr/ZONE_VOLUMES[0]) * (0-C_SO3[0]) + rate[0])
...
for i in range(N_ZONES):
    ...
    mpc_graphics.add_line(var_type='_aux', var_name=f'Cp_o2_{i}', axis=ax[i][0], label='O2')
